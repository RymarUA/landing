...
export function EnhancedShopCatalog({ products }: ShopCatalogProps) {
  const { addItem } = useCart();
  
  const [active, setActive] = useState<string>("Всі");
  const [availableCategories, setAvailableCategories] = useState<string[]>(getStaticCategories());
  ...
  // Асинхронно оновлюємо категорії з кешу
  useEffect(() => {
    let isMounted = true;
  ...
  // Синхронізація з URL (query params + hash)
  useEffect(() => {
    let isMounted = true;
    ...
    const syncFromURL = () => {
      ...
      if (sortParam && SORT_OPTIONS.some(opt => opt.value === sortParam)) {
        setSortKey(sortParam as SortKey);
      }
