...
/* ─────────────────────────────────────────────────────────────────────────
   REAL INSTAGRAM FETCH (commented out — enable when ready)
   ───────────────────────────────────────────────────────────────────────── */

/**
 * Fetches recent media from the Instagram Graph API and parses captions
 * into CatalogProduct objects.
 *
 * TO ENABLE:
 * 1. Set INSTAGRAM_ACCESS_TOKEN and INSTAGRAM_USER_ID in .env.local
 * 2. Uncomment this function
 * 3. In getCatalogProducts(), replace `return MOCK_PRODUCTS` with
 *    `return fetchInstagramProducts()`
 *
 * NOTE: Long-lived tokens expire after 60 days — set up a cron job to
 * refresh them via GET /refresh_access_token.
 */
/*
async function fetchInstagramProducts(): Promise<CatalogProduct[]> {
  const token = process.env.INSTAGRAM_ACCESS_TOKEN;
  const userId = process.env.INSTAGRAM_USER_ID;

  if (!token || !userId) {
    console.warn("[instagram-catalog] Credentials not set — falling back to mock");
  ...
  const res = await fetch(url, {
    next: { revalidate: 3600 }, // ISR: revalidate catalog every 1 hour
  });

  if (!res.ok) {
    const err = await res.text();
    console.error("[instagram-catalog] Graph API error:", err);
    // Graceful fallback: return mock data instead of crashing
    return MOCK_PRODUCTS;
  }

  const data: InstagramMediaResponse = await res.json();
  return data.data.map((item, i) => parseInstagramCaption(item, i));
}
*/
 ...
 *
 * Currently uses mock data. To switch to live Instagram feed:
 *   1. Uncomment `fetchInstagramProducts()` above.
 *   2. Replace `return MOCK_PRODUCTS` below with `return fetchInstagramProducts()`.
 */
export async function getCatalogProducts(): Promise<CatalogProduct[]> {
  // ── MOCK (active) ──────────────────────────────────────────────────────
  return MOCK_PRODUCTS;

  // ── REAL Instagram API (swap in when credentials are ready) ───────────
  // return fetchInstagramProducts();
}
